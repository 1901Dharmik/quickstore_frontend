...
export function NewArrivalSection() {
  // Using is_featured as a proxy since newArrival isn't a direct filter in the Adonis backend
  const { data: res, isPending, isError } = useQuery({
    queryKey: ['products', 'new_arrivals'],
    queryFn: () => fetchProducts({ is_new_arrival: true, limit: 8 }),
  });
  ...
  return (
    <ExploreCarouselSection
      title="New Arrivals"
      description="Be the first to explore our latest collection additions, introducing cutting-edge aesthetics and movements."
      cards={cards}
      viewAllHref="/shop"
    />
  );
